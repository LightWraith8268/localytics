...
function renderReport() {
  ...
  // Pre-populate items filter dropdown immediately with loaded data
  prepopulateItemsFilterDropdown();
...
function renderClientTrackingView() {
  ...
  renderSortableClickableTable(tableEl, ['rank','client','orders','quantity','revenue','cost','profit','margin'], clients.map((c, idx) => ({
    rank: idx + 1,
    client: c.label || 'Unassigned',
    orders: c.orders,
    quantity: c.quantity,
...
function renderItemTrackingView() {
  ...
  renderSortableTable(tableEl, ['rank','item','quantity','revenue','cost','profit','margin'], items.map((item, idx) => ({
    rank: idx + 1,
    item: item.item || 'Unassigned',
    quantity: item.quantity,
    revenue: item.revenue,
...
function canonicalizeItemName(raw) {
  ...
  s = s.replace(/\btri[-\s]?color\b/gi, 'Tri Color')
       ...
       .replace(/clear\s+creek\s+3[-"'\s]*15[-"'\s]*/gi, 'Clear Creek 3"-15"')  // handles: 3-15, 3"15, 3'15, 3"-15", 3"-15', 3'15", 3-15", 3'-15, etc. (outputs with inches markers on both sides)
  ...
  // Also preserve hyphenated words with uppercase letters like Safe-T-Post
  ...
  if (!hasQuotedNumbers) {
    s = s.split(' ').map(w => {
      ...
      if (/-[A-Z]/.test(w)) return w; // keep hyphenated words with uppercase like Safe-T-Post as-is
